...
import { validateStudentInList, getStudentByHallTicket } from "@/services/studentsListService";
...
export const verifyStudentInDepartment = async (
  ...
  try {
    ...
    const isInStudentsList = await validateStudentInList(hallTicket, studentName, year);
    ...
    return {
      isValid: false,
      source: 'none',
      message: 'Student not found in any verification source. Please check Hall Ticket Number, Name, and Year.',
    };
 ...
 */
export const quickStudentCheck = async (hallTicket: string): Promise<boolean> => {
  try {

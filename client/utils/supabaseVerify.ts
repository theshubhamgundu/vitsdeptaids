...
export const verifySupabaseCredentials = async () => {
  ...
  try {
    ...
    if (tablesError) {
      console.log("⚠️ Tables query failed (this is expected if RLS is strict):", tablesError.message);
      ...
      if (testError) {
        if (testError.message.includes('relation "student_data" does not exist')) {
          console.log("📋 Database accessible but student_data table doesn't exist");
          console.log("💡 You need to run the SQL scripts to create the tables");

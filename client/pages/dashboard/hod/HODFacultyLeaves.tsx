import DashboardLayout from "@/components/layout/DashboardLayout";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Construction } from "lucide-react";

const HODFacultyLeaves = () => {
  return (
    <DashboardLayout userType="hod" userName="Dr. Priya Sharma">
      <div className="flex items-center justify-center min-h-[60vh]">
        <Card className="w-full max-w-md text-center">
          <CardHeader>
            <div className="flex justify-center mb-4">
              <Construction className="h-12 w-12 text-purple-600" />
            </div>
            <CardTitle>Faculty Leave Management</CardTitle>
            <CardDescription>Approve and manage faculty leave requests</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-gray-600">
              Review faculty leave applications, approval workflow, and leave balance tracking.
            </p>
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
  );
};

export default HODFacultyLeaves;

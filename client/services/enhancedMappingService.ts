...
class EnhancedMappingService {
  ...
  async getAllStudentsData(): Promise<Student[]> {
    try {
      ...
      const formattedRegisteredStudents: Student[] = registeredStudents.map(student => ({
        id: student.id,
      ...
      const formattedDepartmentStudents: Student[] = departmentStudents.map(student => ({
      ...
      const existingHallTickets = formattedRegisteredStudents.map(s => s.hallTicket);
  ...
  private async saveMappings(mappings: StudentFacultyMapping[]): Promise<void> {
    try {
      ...
      if (supabase) {
        ...
        console.log('Mappings saved to localStorage. Database sync would happen here.');
      }
    } catch (error) {
  ...
  async getStudentMappings(studentId: string): Promise<StudentFacultyMapping[]> {
    try {
      const mappings = await this.getMappings();
  ...
  async bulkAssignStudents(
    assignments: { studentId: string; facultyId: string; mappingType: 'coordinator' | 'counsellor' }[]
  ): Promise<void> {
    try {
  ...
  async getUnassignedStudents(mappingType: 'coordinator' | 'counsellor'): Promise<Student[]> {
    try {
  ...
  async getAssignmentStats(): Promise<{
    ...
    try {
      ...
      const coordinatorMappings = activeMappings.filter(m => m.mappingType === 'coordinator');
      const counsellorMappings = activeMappings.filter(m => m.mappingType === 'counsellor');
